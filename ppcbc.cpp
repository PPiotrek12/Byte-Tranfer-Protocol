...
using namespace std;
const uint64_t DATA_DATA_PACKET_SIZE = 100; // TODO
...
void receive_CON_ACC_RJT(int socket_fd, struct sockaddr_in server_address, uint8_t *type, uint64_t ses_id, int retransmits, uint8_t prot, uint64_t seq_len) {
...
void receive_RCVD_RJT(int socket_fd, struct sockaddr_in server_address, uint64_t ses_id, int retransmits) {
    while (true) {
        ...
        if (res_type != RCVD && res_type != RJT) {
            if ((ses_id == res_ses_id && res_type == CONACC) || res_type == ACC)
...
void send_one_DATA_packet(int socket_fd, struct sockaddr_in server_address, uint8_t ses_id, uint64_t packet_nr, uint32_t bytes_nr, char *data) {
    char message[DATA_PACKET_SIZE + 21];
    message[0] = DATA;
    memcpy(message + 1, &ses_id, 8);
    memcpy(message + 9, &packet_nr, 8);
    memcpy(message + 17, &bytes_nr, 4);
    mempcpy(message + 21, data, bytes_nr); // TODO: no to jest przeciez zle xd
    send_message(socket_fd, message, sizeof(message), server_address);
}

void receive_ACC_RJT(int socket_fd, struct sockaddr_in server_address, uint8_t *type, uint64_t ses_id, int retransmits, uint64_t packet_nr, uint32_t bytes_nr, char *data) {
...
void send_DATA(int socket_fd, struct sockaddr_in server_address, uint64_t ses_id, char *data, int retransmits, uint8_t prot, uint64_t seq_len) {
    ...
    while (already_sent < seq_len) {
        ...
        uint8_t type;
        receive_ACC_RJT(socket_fd, server_address, &type, ses_id, retransmits, packet_nr, bytes_nr, data);
        if (type == RJT)
...
void udp_client(struct sockaddr_in server_address, char *data, uint64_t seq_len, uint8_t prot) {
    ...
    receive_CON_ACC_RJT(socket_fd, server_address, &type, ses_id, retransmit, prot, seq_len);
    if (type == CONRJT) 
        return;
